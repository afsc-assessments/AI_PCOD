...
# sensitivity analysis: Item M23.2_M
# ============================================================ #
# 
...
#M23_2 base no K timeblock no M timeblock
#Number of samples for CAAL age comp based on number of aged samples
#Number of samples for marginal age comps (survey) based on Hulson ISS
#M fixed at 0.417 (like EBS cod)
#settlement fixed to same as EBS cod (1,1,1,0)

#change the M timeblock to just 2 params. 
# *** 
# 
...
#	 /Users/ingrid.spies/Documents/AI_PCOD/2024/Models/M23.2_base1/M23.2_base1/run
# 
# Features: 
...
Exe_path <- file.path(getwd(), 'Executables','SS_V3_30_21','ss_osx') 
...
Dir_M24_0=file.path(dir_SensAnal, 'M24_0',fsep = fsep)
Dir_M24_1=file.path(dir_SensAnal, 'M24_1',fsep = fsep)
Dir_M24_1_data=file.path(dir_model, 'M24_1_data',fsep = fsep)
Dir_M24_0_data=file.path(dir_model, 'M24_0_data',fsep = fsep)
...
var.to.save <- c(var.to.save, 'Dir_M24_1_data') 
...
#####DO NOT RUN AGAIN
#SOMETHIGN HAPPENED - not sure just do not change the files that are in there now.
...
# ======================= #
# Read in the file
StarterFile <- file.path(Dir_2024,'starter.ss_new', fsep = fsep)
...
# Read in the file
DatFile <-file.path(Dir_2024,'data_echo.ss_new', fsep = fsep)
...
Dat_base$catch[36,]=c(2025, 1, 1, 4676, 0.05) #Just a guess for 2025, also need to extrapolate to full year
...
# clean environment
var.to.save <- c(var.to.save, 'Dat_base')
#rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# This is used for the tv part
#Ctlfile <-file.path(Dir_M24_base_2M,'run/control.ss_new', fsep = fsep)#This one was from M23.3 but then I added in the short matrix for TV growth

#Ctl_base_2M <- SS_readctl(
##  file = Ctlfile,
#  use_datlist = TRUE,
#  datlist = file.path(Dir_M24_base_2M,"data_echo.ss_new", fsep = fsep),
#  verbose = TRUE
#)

Ctlfile <-file.path(Dir_2024,'control.ss_new', fsep = fsep)#This one was from M23.3 but then I added in the short matrix for TV growth
...
# clean environment
var.to.save <- c(var.to.save, 'Ctl_base')
#rm(list = setdiff(ls(), var.to.save))
var.to.save <- ls()
# =======================
...
# Read in the file
ForeFile <- file.path(Dir_2024, 'forecast.ss', fsep = fsep)
