...
Dat_base$endyr=2025
Dat_base$catch[35,4]=4676.0 #Final for 2024
Dat_base$catch[36,]=c(2025, 1, 1, 4934.67, 0.05) #Extrapolated.
Dat_base$CPUE[35,]=c(2025, 7, -2, 0, 0)

#Add 2024 age data to the model.
CAAL2024=read.csv("/Users/ingrid.spies/Documents/WorkDellStuff/Assessments/AI_Cod/2025/CAAL_2024.csv")
colnames(CAAL2024)=colnames(Dat_base$agecomp)
ExtAge2024=c(2024, 7, -2, 0, 0, 1, -1, 0, 1804, 620606, 1766798, 4309802, 7003227, 5689880, 1303105, 796919, 89909, 84605, 0, 0, 0, 0)

agecomp2=rbind(Dat_base$agecomp[1:1160,],CAAL2024,Dat_base$agecomp[1161:1173,],ExtAge2024)
Dat_base$agecomp=agecomp2
...
Dat_base$lencomp[48,7:149]=FLF24
Dat_base$lencomp[49,]=Dat_base$lencomp[48,]
Dat_base$lencomp[49,1]=2025
Dat_base$lencomp[49,7:149]=FLF25
...
#Now run again with the model reading from par file?

# Read in the file
StarterFile <- file.path(Dir_M24_1_data,'run/starter.ss_new', fsep = fsep)
...
Start_base$Compatibility=1
Start_base$depl_basis=6
...
Ctlfile <-file.path(Dir_M24_1_data,'/run/control.ss_new', fsep = fsep)#This one was from M23.3 but then I added in the short matrix for TV growth
...
Ctl_base$last_early_yr_nobias_adj=1975.5
Ctl_base$first_yr_fullbias_adj=1987.8
Ctl_base$last_yr_fullbias_adj=2020.8
Ctl_base$first_recent_yr_nobias_adj=2026.6
Ctl_base$max_bias_adj=0.9039
...
ForeFile <- file.path(Dir_M24_1_data, '/run/forecast.ss_new', fsep = fsep)
Fore_base <-SS_readforecast(
  file = ForeFile,
  version = '3.30',
  verbose = T,
...
replist$derived_quants[which(replist$derived_quants=="SSB_2026"),]$Value/replist$derived_quants[which(replist$derived_quants=="SSB_Virgin"),]$Value
