...
#
Restart_SA_modeldvpt <- function(base.model = NULL,
                                 curr.model = NULL,
                                 files = NULL) {
  mess1 <-
...
written one or several new input files for your new model."
  ...
  if (restart == 'yes') {
    ...
    # modelBase <-
    #   rstudioapi::showPrompt(title, message = mess2, default = "(e.g., 23.sq.0)")
    modelBase <- ifelse(
      is.null(base.model),
      rstudioapi::showPrompt(title, message = mess2, default = "(e.g., 23.sq.0)"),
      base.model
    ...
    if (!is.null(modelBase)) {
      ...
      # modelDvpt <-
      #   rstudioapi::showPrompt(title, message = mess3, default = "(e.g., 23.sq.1)")
      modelDvpt <- ifelse(
        is.null(curr.model),
        rstudioapi::showPrompt(title, message = mess3, default = "(e.g., 23.sq.1)"),
        curr.model
...
update_SA_table <- function(SumUp, dir_SensAnal) {
  ...
  table <- table %>%
    footnote(
      ...
      alphabet = c(
        "The **Folder** is housed in the 'models/Sensitivity_Anal' folder; \n",
        "The scripts are housed in the 'R/Stock_Synthesis/Sensitivity_Anal' folder; \n",
        "The models are housed in the 'models/Sensitivity_Anal/**Folder**' folder"
      ),
      footnote_as_chunk = T
    )
...
#' @param new_model (character string)- name of the model you will develop for
#' this analysis.
#'
#' @author Matthieu Veron
#  Contact: mveron@uw.edu
...
#'    "shortspine_thornyhead_2023/model/Sensitivity_Anal" folder. This folder will
#'    house model input and output files,
#' 2. the "Sensitivity_Analysis_Features.txt" file (in the previously created folder).
#'    This file summarizes the features of the sensitivity analysis
...
NewSensAnal <- function(pool = "",
  ...
  # dir_SensAnal <- ifelse(test = pool == "",
  #                        yes = file.path(dir_model, "Sensitivity_Anal", 
  #                                        fsep = fsep), 
  #                        no = file.path(dir_model, "Sensitivity_Anal", 
  #                                       "Base_Model", fsep = fsep))
  if(pool == ""){
    tmpDirPool <- ""
  ...
  for (m in 1:dim(Modcomp)[1]) {
    if (!Modcomp$base_model[m] %in% Models_SA$`Model name`) {
      ...
      cat(
        ...
        Modcomp$new_model[m],
        " does not exist yet. This means you will have to start by creating this base model (",
 ...
 analysis. Please set another name.\n",
  ...
  for (m in 1:length(new_model)) {
    ...
    pathMod <-
      c(pathMod,
        file.path("models", "Sensitivity_Anal", folder_name, new_model[m]))
    } else if (pool == "base_model"){
      pathMod <-
        c(pathMod,
          file.path("models", "Sensitivity_Anal", "Base_Model", folder_name, new_model[m]))
    } else if (pool == "star_panel"){
      pathMod <-
        c(pathMod,
          file.path("models", "Sensitivity_Anal", "STAR_Panel", folder_name, new_model[m]))
    }
 ...
 base model to get its 'data_echo.ss_new' file.\n"
      ...
      cat(
        "\n\n !!! The SS input files have not been copied in the folder of the model: ",
        Modcomp$new_model[m],
  ...
  while (is.null(Features)) {
    Impl <-
      readline(prompt = "Please describe the general features of this sensitivity analysis:")
    Features <- Impl
    Sys.sleep(0.1)
  }
  ...
  cat(
    "Please provide a short description for each model you will implement.
...
# model outputs are stored in the '/run/' folder housed in that directory.\n"
      ...
      if (ModcompWARNING) {
        ...
        cat(
          "# do(es) not exist yet. This means you'll have to start by developing its/their own base model before creating your new model !!!!\n"
        )
...
# stored in the 'SA_plots' folder housed in the root directory of this sensitivity analysis.\n"
    ...
    cat("dir_model <- file.path(here::here(), 'models', fsep = fsep)\n")
    cat("\n")
    ...
    cat("\n")
    cat("# Useful function\n")
    cat("source(file=file.path(dir_script,'utils','ss_functions.R', fsep = fsep))\n")
    cat(
      "source(file=file.path(dir_script,'utils','sensitivity_analysis_utils.R', fsep=fsep))\n"
    )
    cat("\n")
    cat("# Load the Rdata base with parameters and data for 2023 assessment\n")
    cat(
...
# sensitivity analysis:",
  ...
  if (length(new_model) > 1) {
    ...
    cat("# This analysis has been developed based on the following model: \n")
    cat("#", base_model, "\n")
  }
  cat("# \n")
  cat("# Results are stored in the following folder: \n")
  ...
  if (do_model) {
    for (m in 1:dim(Modcomp)[1]) {
      ...
      tmp <-
        gsub(
          pattern = file.path("models", "Sensitivity_Anal", fsep = fsep),
          replacement = "",
...
# model to this SA.\n")
    ...
    ##    tmp <- tmp[!tmp %in% ""]
    ##    
    ##    if (Modcomp[m, "base_model"] %in% c("13.sq", "23.sq.0")) {
    ##      cat(
    ...
    ##    cat("\n")
    ##    SaveDir <-
    ##      c(SaveDir, paste0("Dir_Base", gsub('\\.', '_', Modcomp[m, "base_model"])))
    ...
    ##      pattern = file.path("models", "Sensitivity_Anal", fsep = fsep),
    ##      replacement = "",
    ##      x = Models_SA[Models_SA$`Model name` == Modcomp[1, "base_model"], "path"]
    ##    )
    ##  tmp <-
    ##    stringr::str_split_fixed(string = tmp,
    ##                             pattern = fsep,
    ...
    ##  if (Modcomp[m, "base_model"] %in% c("13.sq", "23.sq.0")) {
    ##    cat(
    ##      "Dir_Base <- file.path(here::here(),",
    ##      paste0("'", paste(tmp, collapse = "','"), "'"),
    ##      ", 'run',fsep = fsep)\n"
    ##    )
    ##  } else {
    ##    cat(
    ##      "Dir_Base <- file.path(dir_SensAnal,",
    ##      paste0("'", paste(tmp, collapse = "','"), "'"),
    ##      ", 'run',fsep = fsep)\n"
    ##    )
    ##    
    ...
    # for (m in 1:dim(ModcompBase)[1]) {
      ...
      if (ModcompBase[m, "base_model"] %in% c("13.sq", "23.sq.0")) {
        cat(
        ...
        } #else if (pool == "base_model" && ModcompBase[m, "base_model"] == "23.Base.model"){
      #     cat(
      #       paste0("Dir_", gsub('\\.', '_', ModcompBase[m, "base_model"])),
      ...
      #     )
      # }
      else if ((pool == "base_model" && ModcompBase[m, "base_model"] == "23.Base.model") || pool == "star_panel"){
            cat(
              paste0("Dir_", gsub('\\.', '_', ModcompBase[m, "base_model"])),
              " <- file.path(dirname(dir_SensAnal), ",
              paste0("'", paste(tmp, collapse = "', '"), "'"),
    ...
    for (m in 1:dim(Modcomp)[1]) {
      if (m == 1) {
        ...
        cat("SA_path <- file.path(dir_SensAnal,'",
            folder_name,
            "', fsep = fsep)",
            sep = "")
        cat("\n")
      ...
      cat(
        ...
        if(pool == "base_model"){
          tmp <-
            gsub(
              pattern = file.path("models", "Sensitivity_Anal", "Base_Model", fsep = fsep),
              replacement = "",
              x = Models_SA[Models_SA$`Model name` == Modcomp[m, "new_model"], "path"]
            )
        } else {
          tmp <-
            gsub(
              pattern = file.path("models", "Sensitivity_Anal", "STAR_Panel", fsep = fsep),
              replacement = "",
              x = Models_SA[Models_SA$`Model name` == Modcomp[m, "new_model"], "path"]
            )
        }
...
#   
# } 
Add_Newmodel <- function(pool = NULL, 
                         SA_ID = NULL,
                         new_model = NULL,
                         base_model = NULL,
                         object = NULL){
...
\t => Refer to the 'Models_Sensitivity_analysis.pdf' for example."
  ...
  environment(Append_SA_analyses_Script) <- environment()
  # Append_SA_analyses_Script(out = out, stp = stp, length_old = length_old)
  Append_SA_analyses_Script(out = out, stp = stp, length_old = length_old, pool = pool)
  cat(
    "- The",
    paste0("'", script_model, "'"),
    " file has been updated in the",
